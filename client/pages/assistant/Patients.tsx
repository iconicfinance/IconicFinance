...
import { searchPatients, getPatientTransactions, Patient, PatientTransaction } from '@/services/patients';
...
export default function AssistantPatients() {
  ...
  const selectPatient = async (patient: Patient) => {
    ...
    setTxLoading(true);
    try {
      const data = await getPatientTransactions(patient.id);
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <p className="text-muted-foreground text-sm font-mono">{selectedPatient.patient_code}</p>
            </div>
            {!txLoading && (
